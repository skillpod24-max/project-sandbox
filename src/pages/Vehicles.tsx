  ...
  const { data: pageData, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [vehiclesRes, vendorsRes, imagesRes, docsRes, settingsRes] = await Promise.all([
        ...
        supabase.from("documents").select("id, reference_id, document_name, document_type, document_url, status, expiry_date, user_id, created_at").eq("reference_type", "vehicle").eq("user_id", user.id),
